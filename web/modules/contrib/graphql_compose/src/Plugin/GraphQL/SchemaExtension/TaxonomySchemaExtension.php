...
class TaxonomySchemaExtension extends SdlSchemaExtensionPluginBase {
  ...
  protected function addQueryFields(ResolverRegistryInterface $registry, ResolverBuilder $builder) {
    ...
    foreach ($taxonomyVocabularies as $taxonomyVocabulary) {
      ...
      $registry->addFieldResolver(
        'Query',
        $taxonomyVocabulary['type_plural'],
        $builder->produce('query_taxonomy_vocabulary:'.$taxonomyVocabulary['id'])
  ...
  protected function addTypeFields(ResolverRegistryInterface $registry, ResolverBuilder $builder) {
    ...
    // foreach ($contentTypes as $contentType) {
      foreach ($taxonomyVocabularies as $taxonomyVocabulary) {  
      foreach ($taxonomyVocabulary['fields'] as $field) {
        $builders = [];
  ...
  protected function loadDefinitionFile($type) {
    return \Drupal::service('graphql_compose.datamanager')->getSdlByStorage('taxonomy_term', $type);
  }
