...
namespace Drupal\Tests\graphql_compose\Kernel;
...
/**
 * Base class for GraphQL Compose GraphQL tests.
 *
 * Provides utility methods for testing GraphQL Compose GraphQL endpoints. Ensures
 * the GraphQL Compose GraphQL server is loaded and configured.
 */
abstract class SocialGraphQLTestBase extends GraphQLTestBase {
  ...
  protected function setUp() : void {
    ...
    // contains the GraphQL Compose GraphQL server.
    $this->installConfig("graphql_compose");

    // Set up the schema and use the GraphQL Compose GraphQL server in queries.
    $server = Server::load("graphql_compose_graphql");
    self::assertInstanceOf(ServerInterface::class, $server, "Test set-up failed: 'graphql_compose_graphql' server is not installed.");
    $this->server = $server;
  }
